...
const logger = createLogger('Yunara:Niu:Download');

export class NiuDownload extends plugin {
  constructor() {
    super({
      name: '咕咕牛图库下载',
      dsc: '下载咕咕牛图库',
  ...
  DownloadTuKu = async (e) => {
    ...
    const cooldownKey = `Yunara:Niu:${commandName}:${e.user_id}`;
    ...
    try {
      ...
      await e.reply(`检测到 ${reposToDownload.length} 个新仓库需要下载，任务已在后台开始，请耐心等待...`, true);

      const downloadPromises = reposToDownload.map(repo => {
        logger.info(`[${repo.description}] 的下载任务已开始...`);
 ...
 const templatePath = path.join(Yunara_Res_Path, 'niu', 'download', 'download.html');
